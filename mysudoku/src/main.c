...
int main(int argc, char *argv[])
{
   ...
   play_mode = GTK_WIDGET(gtk_builder_get_object(builder,"play_mode_toggle_btn"));
...
void on_solve_btn_clicked(){
		...
		for(row = 0; row < 9; row++){
			for(col = 0; col < 9; col++){
				...
				strcpy(buf,gtk_entry_get_text( GTK_ENTRY(wid[row][col]) ));
				if(strlen(buf) > 0){
		...
		if( solve(solver_puzzle) ){
			...
			for(row = 0; row < 9; row++)
				for(col = 0; col < 9; col++)
					if( is_valid(solver_puzzle,row,col,solver_puzzle[row][col]) ){
...
void on_play_mode_toggle_btn_toggled(GtkToggleButton *b){
	...
	if(t){
		...
		char str_time[50];
		char time[5];
		strcpy(str_time,"TIme Taken (in sec) = ");
		for(row = 0; row < 9; row++){
			for(col = 0; col < 9 ; col++){
				...
				if( i != play_puzzle[row][col]){
					...
					//printf("%ld\n",time_taken);
					if(time_taken > 0){
						sprintf(time,"%ld",time_taken);
						strcat(str_time,time);
						gtk_label_set_text(GTK_LABEL(label2),str_time);
					}
		...
		sprintf(str_time,"%ld",time_taken);
					gtk_label_set_text(GTK_LABEL(label2),str_time);
		}
	}
...
void on_hint_clicked(){
	gboolean t = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(play_mode));
	if(t){
		int row, col;
		char buf[2],temp[2];
		for(row = 0; row < 9; row++){
			for(col = 0; col < 9 ; col++){
				...
				if( !(strlen(buf) > 0) ){
					sprintf(temp,"%d", play_puzzle[row][col]);
					gtk_entry_set_text( GTK_ENTRY(wid[row][col]), temp );
